...
namespace LibroWeb.Controllers
{
	...
	public class HomeController : Controller
	{
		public ActionResult Index()
		{
			...
			ModelLayer.PaginationDTO paginaDTO = new ModelLayer.PaginationDTO
			{
				CantidadElementos = modelLibro.Libros.Count,
				SeleccionElementos = 5,
				Libros = modelLibro.Libros,
			};
		...
		{
			ModelLayer.ResultDTO modelDTO = new ModelLayer.ResultDTO();
			ModelLayer.Libro libros = GetLibros(null);
			modelDTO.Objects = new List<object>();
			if (libros.Libros.Count > 0)
			{
				foreach (var item in modelDTO.Objects)
				{
					modelDTO.Objects.Add(JsonConvert.DeserializeObject<ModelLayer.Libro>(item.ToString()));
				}
